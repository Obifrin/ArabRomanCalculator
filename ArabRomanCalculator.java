...
class ThatRoman {
    ...
    boolean MyRoman(String number){
        return KeyMap.containsKey(number.charAt(0));
    }
}


public class ArabRomanCalculator {
        public static void main(String[] args) throws Exception {
            ...
            ThatRoman ThatRoman = new ThatRoman();
            String[] action = {"-","+","/","*"};
            ...
            if(actionIndex == 100){
                try {
                    ...
                    System.out.println("Незадекларированная математичесая операция");
                }
                return;
            }
            String [] b = a.split(DanglingQuantifier[actionIndex]);
            if(b.length !=2)throw new Exception("Только 2 целочисленных значения");
            if(!(ThatRoman.MyRoman(b[0]) == ThatRoman.MyRoman(b[1]))){
                try {
                    throw new IOException();
                }catch (IOException e) {
                    System.out.println("Значения должны быть из одной системы счисления");
                }
                return;
            }else{
                boolean Roman = ThatRoman.MyRoman(b[0]);
                if(Roman){
                    ...
                    int j = mapa.get(b[0]);
                    int k = mapa.get(b[1]);
                    if(!(j > 0 && j < 11) || !(k > 0 && k < 11)){
                        try {
                            throw new IOException();
                        }catch (IOException e) {
                            System.out.println("Введите число от 1 до 10(от I до X)");
                            return;
                        }
                        }else{
                        int jk;
                        switch(actionIndex){
                            case 0:
                                ...
                                if(jk < 1){try {
                                    ...
                                    System.out.println(mapa2.get(jk));}
                                break;
                            case 1:
                                jk = j+k;
                                System.out.println(mapa2.get(jk));
                        ...
                        int o = Integer.parseInt(b[1]);
                        if(!(l > 0 && l < 11) || !(o > 0 && o < 11)){
                            try {
                                throw new IOException();
                            }catch (IOException e) {
                                System.out.println("Введите число от 1 до 10(от I до X)");
